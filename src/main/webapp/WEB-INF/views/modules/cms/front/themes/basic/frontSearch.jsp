	...
	<dl class="search">
		<c:if test="${empty t || t eq 'article'}">
			<c:forEach items="${page.list}" var="article">
				<dt><a href="${ctx}/view-${article.category}-${article.id}${urlSuffix}" class="title" target="_blank">${article.name}</a></dt>
				<dd>${article.content}<span class="info"><br/> &nbsp; 发布时间：${article.publishDate}</span>
					&nbsp;&nbsp;<a href="${ctx}/view-${article.category}-${article.id}${urlSuffix}" target="_blank">查看全文</a><br/></dd>
			</c:forEach>
		</c:if>
