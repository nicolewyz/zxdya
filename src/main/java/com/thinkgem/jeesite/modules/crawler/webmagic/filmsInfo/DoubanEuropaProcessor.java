 ...
 *
 */
public class DoubanEuropaProcessor implements PageProcessor{
    ...
    public void process(Page page) {
        ...
        //1. 如果是用户列表页面 【入口页面】，将所有用户的详细页面的url放入target集合中。
    							https://www.douban.com/doulist/13716059/?start=0&sort=seq&sub_type=
        if(page.getUrl().regex("https://www\\.douban\\.com/doulist/13716059/\\?start=\\d+&sort=seq&sub_type=").match()){
            ...
            List<String> comments = page.getHtml().xpath("//div[@class='ft']//blockquote[@class='comment']/text()").all();
            
            //`namezh`,`nameothers`,`onyear`,`rate`,`rate_users`,`img_url`,`publishTime`,`type`,`movie_info`,`comment`
            for(int i=0; i<titles.size(); i++){
            	try {
            		System.out.println(page.getUrl() +"-----"+titles.get(i)+"========="+rateds.get(i));
    				ygdyDao.insertDoubanTop(
    						titles.get(i).trim().split(" ")[0],//中文名
    						titles.get(i).trim().split(" ").length>1 ? titles.get(i).trim().substring(titles.get(i).trim().split(" ")[0].length()+1)
    								: "",//其他名称
    						years.get(i), 
    ...
    public static void main(String[] args) {
        ...
        Spider.create(new DoubanEuropaProcessor()).addUrl(
        		"https://www.douban.com/doulist/13716059/?start=0&sort=seq&sub_type=")
        .thread(1).run();
